<x-sigem-layout title="Editar Aluno">
    ...
    <form method="POST" action="{{ route('pessoas.alunos.update', $aluno) }}">
        ...
        @php
            ...
            $respPessoa     = $respPrincipal?->usuario;
        ...
        <x-form-card title="Dados do Aluno">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div class="md:col-span-3">
                    <x-form-field label="Município (rede municipal)" name="municipio_id" required hint="Município ao qual o aluno está vinculado na rede; usado em matrículas e filtros.">
                        <select name="municipio_id"
                            ...
                            @foreach($municipios as $municipio)
                                <option value="{{ $municipio->id }}" @selected(old('municipio_id', $aluno->municipio_id) == $municipio->id)>{{ $municipio->nome }}/{{ $municipio->uf }}</option>
                            @endforeach
                        </select>
                    </x-form-field>
                </div>
