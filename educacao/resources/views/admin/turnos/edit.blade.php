<x-sigem-layout title="Editar Turno">
    <x-page-header :title="'Editar: ' . $turno->nome" :back-route="route('admin.turnos.index')" back-label="Voltar"/>
        ...
        <x-form-card title="Dados do Turno">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                <x-form-field label="Nome" name="nome" required>
                    <input type="text" name="nome" value="{{ old('nome', $turno->nome) }}"
                ...
                <x-form-field label="Hora Início" name="hora_inicio">
                    <input type="time" name="hora_inicio" value="{{ old('hora_inicio', $turno->hora_inicio) }}"
                ...
                <x-form-field label="Hora Fim" name="hora_fim">
                    <input type="time" name="hora_fim" value="{{ old('hora_fim', $turno->hora_fim) }}"
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none text-sm">
                </x-form-field>
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="ativo" id="ativo" value="1" @checked(old('ativo', $turno->ativo))
