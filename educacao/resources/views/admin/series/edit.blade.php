<x-sigem-layout title="Editar Série">
    <x-page-header :title="'Editar: ' . $serie->nome" :back-route="route('admin.series.index')" back-label="Voltar"/>
    <form method="POST" action="{{ route('admin.series.update', $serie) }}">
        @csrf @method('PATCH')
        <x-form-card title="Dados da Série">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-form-field label="Nome" name="nome" required>
                    <input type="text" name="nome" value="{{ old('nome', $serie->nome) }}"
                ...
                <x-form-field label="Etapa de Ensino" name="etapa_ensino_id" required>
                    <select name="etapa_ensino_id" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none text-sm">
                        @foreach($etapas as $etapa)
                            <option value="{{ $etapa->id }}" @selected(old('etapa_ensino_id', $serie->etapa_ensino_id) == $etapa->id)>{{ $etapa->nome }}</option>
                        @endforeach
                    </select>
                </x-form-field>
                <x-form-field label="Sigla" name="sigla">
                    <input type="text" name="sigla" value="{{ old('sigla', $serie->sigla) }}" maxlength="10"
                ...
                <x-form-field label="Ordem" name="ordem">
                    <input type="number" name="ordem" value="{{ old('ordem', $serie->ordem) }}" min="1"
                        class="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none text-sm">
                </x-form-field>
                <div class="flex items-center gap-3">
                    <input type="checkbox" name="ativo" id="ativo" value="1" @checked(old('ativo', $serie->ativo))
