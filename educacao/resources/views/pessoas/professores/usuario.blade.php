<x-sigem-layout :title="'Usuário do Professor'">
    ...
    @if(! $professor->usuario)
        <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            <strong>Cadastro incompleto:</strong> não há registro de usuario vinculado a este professor. Não é possível vincular login.
        </div>
    @endif

    @if($usuarioVinculado && $usuarioVinculado->password)
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-6">
            <h3 class="font-semibold text-gray-800 mb-3">Login já cadastrado</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    <p class="text-xs text-gray-500 uppercase mb-0.5">Nome</p>
                    <p class="font-medium text-gray-900">{{ $usuarioVinculado->nome }}</p>
                </div>
                <div>
    ...
    @else
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm p-6 max-w-2xl">
            ...
            <p class="text-sm text-gray-500 mb-5">
                Este formulário define as credenciais de acesso (e-mail, login e senha) no usuário deste professor e garante o perfil <strong>professor</strong>.
            </p>
