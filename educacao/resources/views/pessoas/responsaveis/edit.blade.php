<x-sigem-layout title="Editar Responsável">
    ...
    <form method="POST" action="{{ route('pessoas.responsaveis.update', $responsavel) }}">
        @csrf @method('PATCH')
        @php
            $pessoaResp = $responsavel->usuario;
        ...
        <x-form-card title="Dados Pessoais e de Contato">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <x-form-field label="Tipo de Responsável" name="tipo_responsavel" required>
                    <select name="tipo_responsavel" class="w-full px-4 py-2.5 rounded-xl border border-gray-300 focus:ring-2 focus:ring-indigo-500 outline-none text-sm">
                        @foreach(['Mãe', 'Pai', 'Avó', 'Avô', 'Tio(a)', 'Tutor(a)', 'Cônjuge', 'Outro'] as $tipo)
                            <option value="{{ $tipo }}" @selected(old('tipo_responsavel', $responsavel->tipo_responsavel) === $tipo)>{{ $tipo }}</option>
                        @endforeach
                    </select>
                </x-form-field>
                ...
                <div class="flex flex-col gap-3">
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="responsavel_legal" id="responsavel_legal" value="1" @checked(old('responsavel_legal', $responsavel->responsavel_legal))
                    ...
                    <div class="flex items-center gap-3">
                        <input type="checkbox" name="recebe_notificacao" id="recebe_notificacao" value="1" @checked(old('recebe_notificacao', $responsavel->recebe_notificacao))
                            class="w-4 h-4 rounded text-indigo-600 border-gray-300 focus:ring-indigo-500">
