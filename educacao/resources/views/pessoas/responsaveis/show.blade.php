<x-sigem-layout :title="$responsavel->nome">
    <x-page-header :title="$responsavel->nome" subtitle="Ficha do Responsável" :back-route="route('pessoas.responsaveis.index')" back-label="Voltar">
        <x-slot name="actions">
    ...
    @if(! $responsavel->usuario)
        <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            <strong>Cadastro incompleto:</strong> não há registro de usuario vinculado. Corrija o vínculo (<code class="text-xs">pessoa_id</code>) no banco de dados.
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div class="lg:col-span-2 space-y-5">
            <div class="bg-white rounded-2xl border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><p class="text-xs text-gray-500 uppercase mb-0.5">CPF</p><p class="font-medium">{{ $responsavel->usuario?->cpf ?? '—' }}</p></div>
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5 h-fit">
            ...
            <div class="space-y-2">
                @if($responsavel->usuario)
                    @forelse($responsavel->usuario->contatos as $contato)
                        <div>
