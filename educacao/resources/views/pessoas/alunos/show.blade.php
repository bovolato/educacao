<x-sigem-layout :title="$aluno->nome">
    ...
    @if(! $aluno->usuario)
        <div class="mb-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            <strong>Cadastro incompleto:</strong> não há registro de usuario vinculado a este aluno. Os dados pessoais não podem ser exibidos. Corrija o vínculo (<code class="text-xs">pessoa_id</code>) no banco de dados.
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2 space-y-5">
            ...
            @if($aluno->usuario && $aluno->usuario->contatos->isNotEmpty())
                <div class="bg-white rounded-2xl border border-gray-200 p-6">
                    <h3 class="font-semibold text-gray-800 mb-4">Contatos</h3>
                    <div class="space-y-2">
                        @foreach($aluno->usuario->contatos as $contato)
            ...
            @if($aluno->usuario && $aluno->usuario->enderecos->isNotEmpty())
                @php $end = $aluno->usuario->enderecos->firstWhere('principal', true) ?? $aluno->usuario->enderecos->first(); @endphp
                <div class="bg-white rounded-2xl border border-gray-200 p-6">
                                            ...
                                            <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-2">
                                                <div>
                                                    ...
                                                    <p class="text-xs text-gray-500 mt-0.5">
                                                        {{ $an->professor?->usuario?->nome ?? 'Professor' }}
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-5">
                ...
                <div class="space-y-3">
                    <div><p class="text-xs text-gray-500 uppercase mb-0.5">Município (rede)</p><p class="font-medium text-sm">{{ $aluno->municipio?->nome ?? '—' }}</p></div>
