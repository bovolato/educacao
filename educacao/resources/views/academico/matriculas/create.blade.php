    ...
    <form method="POST" action="{{ route('academico.matriculas.store') }}"
        ...
        <x-form-card title="Dados da Matrícula">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                @else
                <x-form-field label="Município" name="municipio_filtro" hint="Filtra alunos e escolas deste município. Em branco, lista todos.">
                    <select id="municipio_filtro" x-model="municipioSelecionado" @change="onMunicipioChange()"
                ...
                <x-form-field label="Escola" name="escola_id" required>
                    <select name="escola_id" x-model="escolaId" @change="carregarTurmas()"
                        :disabled="municipioSelecionado && escolasFiltradas.length === 0"
                        ...
                        <option value="">{{ $municipios->isEmpty() ? 'Cadastre municípios' : 'Selecione a escola...' }}</option>
        ...
        function matriculaForm() {
            ...
            return {
                ...
                get alunosFiltrados() {
                    ...
                    return alunosAll.filter(a => String(a.municipio_id || '') === String(this.municipioSelecionado));
                },
                get escolasFiltradas() {
                    if (!this.municipioSelecionado) {
                        return escolasAll;
                    }
                    const base = escolasAll.filter(e => String(e.municipio_id || '') === String(this.municipioSelecionado));
                ...
                onMunicipioChange() {
                    const filtradasEscolas = this.municipioSelecionado
                        ? escolasAll.filter(e => String(e.municipio_id || '') === String(this.municipioSelecionado))
                        : escolasAll;
                ...
                init() {
                    if (this.escolaId) {
                        ...
                        if (found && found.municipio_id) {
                            this.municipioSelecionado = String(found.municipio_id);
                        }
                        this.carregarTurmas();
                    } else if (this.alunoId) {
                        const al = alunosAll.find(a => String(a.id) === String(this.alunoId));
                        if (al && al.municipio_id) {
                            this.municipioSelecionado = String(al.municipio_id);
                        }
                    }
                },
