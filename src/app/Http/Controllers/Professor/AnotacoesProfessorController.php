...
class AnotacoesProfessorController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = AnotacaoProfessor::query()
            ...
            ->with(['turma', 'matricula.aluno.pessoa'])
    ...
    public function create(Request $request)
    {
        ...
        if ($turmaId) {
            ...
            $alunosOptions = Matricula::query()
                ...
                ->select('matriculas.*')
                ->get()
                ->mapWithKeys(fn ($m) => [(int) $m->id => ($m->aluno?->pessoa?->nome ?? '—')])
                ->toArray();
        }

        if ($matriculaId) {
            $matricula = Matricula::with(['turma', 'aluno.pessoa'])->findOrFail($matriculaId);
            ...
            $alunosOptions = $alunosOptions ?: [$matricula->id => ($matricula->aluno?->pessoa?->nome ?? '—')];
        }
    ...
    public function show(AnotacaoProfessor $anotacaoProfessor)
    {
        ...
        $anotacaoProfessor->load(['turma', 'matricula.aluno.pessoa']);
